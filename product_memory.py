"""
Shared ProductMemory management for both main.py and streamlit_app.py
"""
...
class ProductMemoryManager:
    ...
    def update_with_previous(self, search_result: str) -> None:
        ...
        try:
            ...
            if products and products[0]:
                ...
                # Try to infer product_type, brand from search results
                # For now just store that we had a successful search
                if self.verbose:
                    pass
    ...
    def merge_memory(self, new_memory: Optional[ProductMemory] | dict) -> ProductMemory:
        ...
        - previous_memory fields not in new_memory → preserve
        """
        if new_memory is None:
            new_memory = ProductMemory()
        
        if isinstance(new_memory, dict):
            new_memory = ProductMemory(**{k: v for k, v in new_memory.items() 
                                         if k in ['product_type', 'brand', 'series', 'model', 'cpu', 
        ...
        # Merge: take non-None from new, fallback to previous
        merged_dict = {}
        for field in ['product_type', 'brand', 'series', 'model', 'cpu', 'ram', 
                     'storage', 'capacity', 'interface', 'price_range', 'product_link']:
            new_val = getattr(new_memory, field, None)
            prev_val = getattr(self.previous_memory, field, None)
            # Use new if not None, else use previous if it exists
        ...
        if self.verbose:
            new_log = {k: v for k, v in asdict(new_memory).items() if v}
            prev_log = {k: v for k, v in asdict(self.previous_memory).items() if v}
            merged_log = {k: v for k, v in asdict(merged).items() if v}
    ...
    def get_context_message(self) -> Optional[SystemMessage]:
        ...
        This helps LLM merge/preserve fields when user asks follow-up questions.
        """
        if not self.previous_memory:
            return None
        ...
        # Format previous memory as readable context
        prev_fields = ", ".join([f"{k}='{v}'" for k, v in prev_dict.items()])
        
        context = f"""Lưu ý về lượt hội thoại trước:
Thông tin sản phẩm từ lượt trước: {prev_fields}
